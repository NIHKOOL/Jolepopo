...
public class GameScene extends AnimationTimer implements Updatable {
	...
	private final String[] backgrounds = { "map/BG_1.png", "map/terrace.png", "map/BG_2.png", "map/terrace.png",
			"map/BG_4.png" };
	...
	public void start(Scene scene) {
		...
		scene.setOnKeyPressed(e -> {
			...
			if (code == KeyCode.SPACE || code == KeyCode.W)
				currentPlayer.jump();
			if (code == KeyCode.K)
				currentPlayer.abilityOne();
			if (code == KeyCode.L)
				currentPlayer.abilityTwo();
			if (code == KeyCode.TAB)
				switchCharacter();
			if (code == KeyCode.ENTER && canChangeMap)
				changeMap();
			if (code == KeyCode.E)
				goToStopScene();
	...
	public void update() {
		...
		if (currentMapIndex == 1 || currentMapIndex == 3) {
			...
			canChangeMap = currentPlayer.getX() >= GameConfig.MAP_WIDTH - 100
					&& (logicManger.getMonsters().size() == 0);
		}
		if (bonfire != null)
			bonfire.update();
	...
	private void render() {
		...
		if (bonfireMenuOpen) {
			...
			gc.fillText("[ " + GameConfig.MANA_REGEN + " >>> " + (GameConfig.MANA_REGEN + 0.3) + " ]",
					canvas.getWidth() / 2 - 60, canvas.getHeight() / 2 - 15);
			gc.fillText("2: ++Boost DMG Multiplier", canvas.getWidth() / 2 - 80, canvas.getHeight() / 2);
			gc.fillText("[ x" + GameConfig.PLAYER_DAMAGE_MULTIPLIER + " >>> "
					+ (GameConfig.PLAYER_DAMAGE_MULTIPLIER + 0.5) + " ]", canvas.getWidth() / 2 - 60,
					canvas.getHeight() / 2 + 15);
		}
		drawDebugMode();
	}
	...
	public void drawDebugMode() {
		if (debugMode) {
			System.out.println("DebugMode / " + "SysTime " + System.currentTimeMillis() + "ms / posX "
					+ currentPlayer.getX() + " posY " + currentPlayer.getY());
			if (currentPlayer instanceof SamuraiMelee) {
				Rectangle2D hitbox = currentPlayer.getAttackBox();
				gc.setStroke(Color.WHITE);
				gc.setLineWidth(2);
				gc.strokeRect(hitbox.getMinX() - camera.getX(), hitbox.getMinY() - camera.getY(), hitbox.getWidth(),
						hitbox.getHeight());
	...
	private void changeMap() {
		...
		if (currentMapIndex == 1) {
			...
			for (int x : skeletonX) {
				monsters.add(new Skeleton(x, GameConfig.GROUND_LEVEL + 20, currentPlayer));
			}
			for (int x : skeletonWarriorX) {
				monsters.add(new SkeletonWarrior(x, GameConfig.GROUND_LEVEL + 20, currentPlayer));
			}
